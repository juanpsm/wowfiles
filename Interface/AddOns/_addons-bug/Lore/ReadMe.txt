...
[1.0]	What is Lore?
 ...
 [1.3]	Accessing Lore's settings window.
...
[3.0]	Advanced settings
 ...
 [3.13]	Speaking a language worse than understanding it
 [3.14]	The Lore Button skin
 [3.15]	The Lore Debugger
...
[1.3]	Accessing Lore's settings window.
	...
	Lore 7.0 has been enriched with an all new graphical settings window, making it a lot easier for you to change
	just that setting you wanted to change, set up your new character to your own wishes, or just get an overview
	of the settings you have set.

	The common way of accessing the settings window is by typing: "/lore"

	Previously this gave a list of Lore commands. That list can now be accessed by typing: "/lore help"
...
[2.1]	Setting up Lore for use
	...
	More settings can be accessed by typing "/lore". A list of commands can be found by typing "/lore help".
...
[3.0]	Advanced settings
	...
	Most of these settings are also available through the graphical settings window, which you can access
	by typing "/lore".
...
[3.5]	Changing left- and right-click functionality
	...
	settings -> open/close the Lore settings window
...
[3.13]	Speaking a language worse than understanding it
	...
	If you want your character to speak a language worse than he or she can understand it (which is quite common
	if you try to speak a foreign language you don't master) you can define this by changing the Language Skill
	Modification.

	You will find a slider for this at the settings window. Please type "/lore" to open the settings window.

	The slider allows you to set a value ranging -100% through +100%. The default is 0% and that equals speaking
	languages as well as you understand them.

	If you apply a negative value, then you will speak the language worse than you can understand it.
	If you apply a positive value, then you will speak the language better than you can understand it.

	The latter is useful if you want to see 'scrambled' text because your character doesn't fully understand
	the language, but you want to be able to type freely and apply your roleplaying skills into making your
	character speak with a handicap.

	NOTE: THIS ONLY APPLIES TO LANGUAGES THAT HAVE A SKILL OF 99% OR SMALLER. THIS DOES NOT APPLY TO LANGUAGES
	      THAT YOUR SKILL IS LISTED AS 100%.

	Example:
	- Your Common language skill is 100%.
	- Your Gnomish language skill is 100%.
	- Your Demonic language skill is 80%.
	- You have set your LANGUAGE MODIFIER to -20%

	This will result into:
	- You understand and speak Common as well as Gnomish for the full 100%
	- You will understand Demonic for 80%, but can only speak it as well as 60% (80-20).

	Don't worry about the numbers in the calculation going below 0 or above 100. Feel free to set the slider at
	any position you want.

[3.14]	The Lore Button skin

	The first time you log in your character with the new Lore version 7.0 (or higher), your Lore Button will
	receive a skin reflecting your character's faction (red for Horde, blue for Alliance). If you want to change
	this skin, you can do so from the Lore Settings window (type "/lore" to open it).

	There is currently a choice between 16 different skins. These skins are saved on a 'per character' base, so
	you can just pick one you like for all your characters, or you can select a different one for each character,
	reflecting their nature or alignment.

[3.15]	The Lore Debugger

	The debugger is a tool that can help tracing problems where they are found... if you have any idea of how
	Lore structurally/technically works (which most people do not). So unless you know what you're doing (or you
	have been asked to "do 'this and that' and tell me which messages come onto your screen") you probably do
	not want to touch this. Be warned, in debug mode Lore generates quite a bunch of spam onto your screen! Don't
	use this during raids or in areas where you may find a lot of (translatable) speech.

	You can switch the Lore Debugger on by typing "/lore debug on" and off by typing "/lore debug off". Simply
	typing "/lore debug" will toggle the function.

	Debug lines all start with "Lore debugger:", contain a three-digit number, followed by some text. The text is
	sometimes information on procedures Lore is going through, or feedback on certain variables. If you want a
	dump of variables you can also get this by typing "/lore dump" (the debug mode has to be switched on for this).

	Dump will give the following variables in this order: locked, screening, autotranslation, effect, visibility,
	to, left, right, current language, lore language, current skill, previous message, last player.

	Generally when speaking, Lore should go through one of the following cycles:
	* Talking in a custom language, when nobody who knows it is nearby:
	  021, 025, 027, 030
	* Talking in a custom language, when one person who knows it is nearby:
	  021, 025, 027, 030, 043, 044, 041
	* Hearing a custom language you fully understand:
	  042, 040
	* Hearing a custom language you partially understand:
	  026, 042, 040, 026
	* Speaking a custom language that you only partially master:
	  021, 025, 027, 026, 030
	* Speaking in your Faction Common language (with 100% skill):
	  021, 030
	* Speaking in your Faction Common language (with skill lower than 100%):
	  021, 029, 026 030
	* Hearing something said in your Faction Common language (if your skill is lower than 100%):
	  026
...
[4.0]	General commands
	...
	Some of these settings are also available through the graphical settings window, which you can access
	by typing "/lore".
...
[4.8]	List of commands
	...
	You can get a list of Lore commands with short descriptions by typing "/lore help".
...
[6.0]	Known issues & troubleshooting
	...
	- Some people sometimes experience that they see words in CAPITALS when they hear a language they only partially
	  understand. Why this happens is still under investigation. Cycling through your Fluencies seems to fix this.
	...
	* Problem: I get a message that the Lore Self-test failed. Why is that, and what do I do?
	* Solution: The self test runs once somewhere in the beginning of your session. It will generally tell you that
	  the auto-translation function is operative and working. If it is not working for some reason, it will tell you
	  after a while. There are several reasons for this. Maybe you couldn't join the Lore channel, or there is another
	  AddOn that is causing conflict with Lore. One of the known AddOns that can still do this is Eloquence. If you
	  have Eloquence, please open its options by typing "/elo" and make sure that the checkbox named "Color names in
	  chat" is UNTICKED. Having this box enabled will cause that Lore no longer understands its own autotrans commands.
...
[7.0]	Acknowledgements
	...
	- Alchmo for providing language information for the German localisation
	- Helwaluin for providing language information for the French localisation
...
[8.0]	Change list
	...
	Lore v7.0 ---
	 ...
	 <> The following settings have been changed to be saved on a 'per character' basis: tablet position, mass translation, stutterfrequency, lock state, screening.

	 <> Special language features (such as stuttering, lisping, hissing) are now saved, also on a per-character basis.

	 <> A graphical settings window has been added. This can be accessed by typing '/lore'.

	 <> The command to get a list of Lore commands has been changed from '/lore' to '/lore help'.

	 <> A new command has been added to Leftclick/rightclick: 'settings'. This opens the Lore Settings window.

	 <> The Lore button now has a skin. For each character separately, one of the 16 skins can be chosen. The Faction default colours are default for each of your characters. If you want something that doesn't jump out much and sticks more to the rest of the default UI skins, 'Shadow' is probably for you.

	 <> The auto-translation self-test no longer runs on entering the world. You can now access it from the options window, and there it will give more info on what went wrong, if something did. Faction Common is used if the tablet is empty.

	 <> A so called 'speech skill modifier' has been added. Available through the settings window. Ranges are -100% through +100%. Example: A value of -20% makes you speak a language you don't master 20% worse than that you can understand it.


	Lore v6.1 ---

	 <> Localisation added for German and French default Blizzard languages. Autotranslation should now work for those.

	 <> Debugger added. If you encounter problems, type "/lore debug on" and try again. The info on your screen will help me (and perhaps even you) see what is the problem.

	 <> New command: /lore dump (part of the debugger, debugger needs to be switched on for this to work). Dumps a string of variable info onto the screen.

	 <> Lore now includes a 'self test', which will run the first time you type something. If you see a message about 'auto-translation' it means that auto-translation works. If you never see anything, there is most likely something blocking auto-translation. (Note, if you run Lore for the very first time, there will be no selftest.)

	 <> Made a small fix in the /language command.

	 <> Updated the readme.txt document to include the new lore commands and some debug info.
