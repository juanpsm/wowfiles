...
local function ScanContainer(bagID, containerType)
	...
	if containerType == GUILDBANK then
		...
		if not guildName then return end		-- to do, add an error message here, should not happen, since a scan is triggered when opening the guild bank
		
		local key = format("%s.%s.%s", THIS_ACCOUNT, GetRealmName(), guildName)
		bag = addon.db.global.Guilds[key].Tabs[bagID]	-- bag is actually the current tab
	else
