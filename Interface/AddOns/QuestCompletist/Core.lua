...
function qcUpdateMenu(zoneID, startItem, noNewZoneQuery)
	...
	for menuIndex = 1, 16 do
		...
		if (questCount >= offsetIndex) then
			_G["qcMenuButton" .. menuIndex .. "_QuestName"]:SetText(qcZoneQuests[offsetIndex][5])
			if (qcShowDebugInformation == true) then
				_G["qcMenuButton" .. menuIndex .. "_QuestTag"]:SetText(qcZoneQuests[offsetIndex][4] .. " | id: " .. qcZoneQuests[offsetIndex][2])
			else
				_G["qcMenuButton" .. menuIndex .. "_QuestTag"]:SetText("")
			end
					...
					if (qcQuestDatabase[6] == 1) or (qcQuestDatabase[6] == 3) or (qcQuestDatabase[6] == 4) then
						if (_G["qcClassCheckButton"]:GetChecked() == 1) then -- We dont want to includes quests that are for other classes --
							if (qcQuestDatabase[7] == 1) then -- This is a class specific quest --
								if (string.find(qcQuestDatabase[8], string.upper(playerEnglishClass)) ~= nil) then -- This quest is for the players class --
...
local dropdownInfo
	...
	if (level == 1) then
		...
		function dropdownInfo.func(button, arg1)
			print(COLOUR_WARLOCK .. "Quest Completist: " .. FONT_COLOR_CODE_CLOSE .. "A server query has been requested. This will likely freeze your game for a few seconds - depending on how many quests you have completed in the past.")
...
local function qcEventHandler(self, event, ...) -- The event handler --
	...
	elseif (event == "QUEST_ACCEPTED") then -- Args: questLogIndex, questID --
		addQuest(...)
