...
function GupPet_Interface_BuildMacro( self, option )
	...
	if GUPPET_TEMPCLASSDATA.Type == "HUNTER" and  option ~= "FLY"  then
		...
		if GUPPET_TEMPCLASSDATA["Pack"]["Usable"] and GetNumPartyMembers() > 0 then
		...
		if GUPPET_SAVEDDATA.SlowGround.Total == 0 and GUPPET_SAVEDDATA.FastGround.Total == 0 then
	...
	if GUPPET_TEMPCLASSDATA.Type == "SHAMAN" and  option ~= "FLY"  then
		...
		-- if you dont have any groundmounts
		if GUPPET_SAVEDDATA.SlowGround.Total == 0 and GUPPET_SAVEDDATA.FastGround.Total == 0 and GUPPET_TEMPCLASSDATA["Ghost"]["Usable"] then
			self:SetAttribute("macrotext"		, "/use "..GUPPET_TEMPCLASSDATA["Ghost"]["Name"] )
...
function GupPet_Interface_DragIngameButtons( option )
	...
	if option == "Reset" then
		...
		_, _, GUPPET_OPTIONS.IngameButton.RelativePoint, GUPPET_OPTIONS.IngameButton.xOffset, GUPPET_OPTIONS.IngameButton.yOffset = GUPPET_INTERFACE_INGAMEBUTTONS.Frame:GetPoint()
...
function GupPet_Interface_IngameButtons()
	...
	if GUPPET_OPTIONS.IngameButton.MountButton == "Auto" and GUPPET_Z["SUPPORT"] and ( GUPPET_SAVEDDATA[ "SlowFly" ]["Total"] > 0 or GUPPET_SAVEDDATA[ "FastFly" ]["Total"] > 0 or (GUPPET_TEMPCLASSDATA.Type == "DRUID" and( GUPPET_TEMPCLASSDATA["FastFly"]["Usable"] or GUPPET_TEMPCLASSDATA["SlowFly"]["Usable"] )) ) then
				
			buttonNr = buttonNr + 1 ;
			_G["GupPet_IngameFrameTemplateAuto"]:Show() ;
	else
		if GUPPET_SAVEDDATA[ "SlowGround" ]["Total"] > 0 or GUPPET_SAVEDDATA[ "FastGround" ]["Total"] > 0 
		...
		if GUPPET_SAVEDDATA[ "SlowFly" ]["Total"] > 0 or GUPPET_SAVEDDATA[ "FastFly" ]["Total"] > 0  
	...
	if GUPPET_OPTIONS.IngameButton.Alignment == "Vertical" then
		...
		_, _, _, xOfs, yOfs = _G["GupPet_IngameFrameTemplateFly"]:GetPoint() ;
		if xOfs and yOfs then
		_G["GupPet_IngameFrameTemplateFly"]:SetPoint("CENTER",yOfs,-xOfs);
		end
		
		_, _, _, xOfs, yOfs = _G["GupPet_IngameFrameTemplateAuto"]:GetPoint() ;
		if xOfs and yOfs then
		_G["GupPet_IngameFrameTemplateAuto"]:SetPoint("CENTER",yOfs,-xOfs);
		end
		
		_, _, _, xOfs, yOfs = _G["GupPet_IngameFrameTemplateCompanion"]:GetPoint() ;
		if xOfs and yOfs then
