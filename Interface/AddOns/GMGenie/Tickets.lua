...
function GMGenie.Tickets.setComment()
    local longitudTexto = strlen(GMGenie_Tickets_View_Comment:GetText());
    local longitudTicket = strlen(GMGenie.Tickets.currentTicket["ticketId"]);
    local longitudComando = 14;
    local total = longitudTexto + longitudTicket + longitudComando;
    if total > 255 then
        message("Superaste los 255 caracteres! No se puede enviar la respuesta");
    else
        SendChatMessage(".tic resp app " .. GMGenie.Tickets.currentTicket["ticketId"] .. " " .. GMGenie_Tickets_View_Comment:GetText(), "GUILD");
	    SendChatMessage(".ticket comple " .. GMGenie.Tickets.currentTicket["ticketId"]);
    end
